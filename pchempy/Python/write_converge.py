...
def write_converge_fortran(runname,atol=0.05,rtol=1.0e-4,max_iter=5000,mindt=1.0e-4):
    
    """
        FUNCTION NAME : write_converge_fortran()
        
        DESCRIPTION : Routine to convergence engine of the photochemical model
        ...
        OPTIONAL INPUTS:
            ...
            mindt :: Minimum timestep (s)
        
        OUTPUTS :
        
        CALLING SEQUENCE:
        
            write_converge_fortran(runname)
...
MODULE {runname}_converge

    real, parameter :: rtol = {rtol}             !relative tolerance
    real, parameter :: atol = {atol}             !absolute tolerance
    integer, parameter :: max_iter = {max_iter}  !maximum number of iterations
    double precision :: mindt = {mindt}          !minimum timestep
    
CONTAINS

!==============================================================================================================================================

    subroutine converge_model_rosenbrock(nlay,ngas,h,P,T,N,gasID,isoID,mmol,A,s,B,&
        ...
        use chemistry
        use photolysis
        use {runname}_diffusion
        use {runname}_chemical_network
        ...
        integer :: gasID_phot(ngas), isoID_phot(ngas)
        ...
        call number_reactions(photolysis_inc,nreactions,n_photolysis)
        ...
        call load_reactions(nlay,ngas,gasID,isoID,h,P,T,N,&
        ...
        if(photolysis_inc)then
            ...
            call load_reactions_photolysis(ngas_phot,gasID_phot,isoID_phot)
        ...
        do while(converged.eqv..false.)
            ...
            if(chemistry_inc)then
                ...
                !Loading the chemical reactions and calculating the reaction rates
                call load_reactions(nlay,ngas,gasID,isoID,h,P,T,Ncur,&
                nreactions,n_photolysis,rrates,rtype,ns,sID,sISO,sf,npr,pID,pISO,pf)
...
!==============================================================================================================================================
    ...
    end subroutine eval_fn_system
    
END MODULE {runname}_converge

"""

    #print(fortran_code)    
    # Open the file in write mode and write the Fortran code
    with open(runname+'_converge.f90', 'w') as f:
